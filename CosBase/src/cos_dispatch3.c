...
enlarge_cache(void)
{
  ...
  n = cos_method_cache3.msk ? (cos_method_cache3.msk+1)*2 : 512;
  cos_method_clearCache3();

  cos_method_cache3.slot = malloc(n * sizeof *cos_method_cache3.slot);
  if (!cos_method_cache3.slot)
    cos_abort("method3_lookup: out of memory");

  for (i = 0; i < n; i++)
    cos_method_cache3.slot[i] = &sentinel;

  cos_method_cache3.msk = n-1;
}
...
load_method(SEL _sel, U32 id1, U32 id2, U32 id3, BOOL load)
{
  ...
  if (!fct) {
    if (!load) return 0;
    fct = cos_method_get3(&COS_GEN_NAME(gunrecognizedMessage3), id1, id2, id3);
    if (!fct)
      cos_abort("method3_lookup: %s not found for classes (%s,%s,%s)",
                "gunrecognizedMessage3",
                cos_class_get(id1)->name, cos_class_get(id2)->name,
                cos_class_get(id3)->name);
  }

  // get slot
  key  = cos_method_hkey3(_sel->Behavior.id,id1,id2,id3);
  slot = cos_method_cache3.slot + (key & cos_method_cache3.msk);

  // get cell
  if (cos_method_cache3.msk < COS_METHOD_MAXSLOT3-1) {
...
#define CACHE_GET_SLOT() \
  ...
  struct cos_method_slot3 **slot = \
    cos_method_cache3.slot + (key & cos_method_cache3.msk);
...
cos_method_clearCache3(void)
{
  ...
  if (cos_method_cache3.slot != &cache_empty) {
    ...
    free(cos_method_cache3.slot);
    cos_method_cache3.slot = &cache_empty;
    cos_method_cache3.msk = 0;
  }
}
